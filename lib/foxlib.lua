            ...
            if player ~= HndlPed then
                ...
                if not sampIsPlayerNpc(playerid) and playerid ~= -1 then 
                    ...
                    for _,player1 in pairs(getAllChars()) do
                        local playerid = select(2, sampGetPlayerIdByCharHandle(player1)) а
                        if not sampIsPlayerNpc(playerid) and playerid ~= -1 then 
...
-- function formatUnixTime(time, format)
    ...
    -- local month = {'€нвар€','феврал€','марта','апрел€','ма€','июн€','июл€','августа','сент€бр€','окт€бр€','но€бр€','декабр€'}
    -- local forms = {
        -- { 'sec', 'секунду', 'секунды', 'секунд' },
        -- { 'min', 'минуту', 'минуты', 'минут' },
        -- { 'hour', 'час', 'часа', 'часов' },
        -- { 'day', 'день', 'дн€', 'дней' },
        -- { 'month', 'мес€ц', 'мес€ца', 'мес€цев' },
        -- { 'year', 'год', 'года', 'лет' }
    -- }
